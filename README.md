...
仓库里的代码经过完整重构与修正：模型内部实现严格对齐论文公式，数据流程消除了信息泄漏，
训练与评测具备可复现性，并内置持续性 / 日周期朴素基线，直接回答"这个模型到底有没有用"。
...
逐步循环在 Python 里极慢（原实现每轮要跑 4×3×96 = 1152 次递归），本项目把它展开成并行形式：
...
| 级别 | 原实现的问题 | 本仓库的处理 |
|---|---|---|
| P0 | `nn.MultiheadAttention` 未设 `batch_first=True`（默认 False），把 batch 当序列、时间步当样本，注意力在**样本之间**计算 | 显式 `batch_first=True`；并加回归测试：同一输入样本的预测不随同批其他样本变化 |
| P0 | 无位置编码，自注意力对时间步置换不变，编码器看不到先后顺序 | 加入可学习（或正弦）位置编码 |
| P0 | xLSTM 把多头维度拍平，`c_t = f·c + i·(v*k)` 用逐元素乘代替外积，矩阵记忆退化为逐元素；归一化分母 `max(abs(n_tᵀ @ q), 1)` 跨样本耦合 | 重写为 `(B, H, d, d)` 矩阵记忆 + 外积更新 + 按 head 归一化，状态形状与论文一致 |
...
| P1 | 预测出现大量负值（原结果 55.4% 为负） | 推理时 `clamp(min=0)`；训练时不截断（否则输出会被永久压在 0 上，梯度消失） |
...
| 原实现（修复前，旧代码保存的结果） | 0.7826 | 1.410 | 1.790 | 0.1309 | 55.02% | 55.4% |

- 相对持续性基线的技能得分 **+0.189**，相对日周期基线 **+0.496**。
- 修复前后用同一份数据、同一套切分与同一口径复算：R² 由 0.783 提升到 0.907，
  RMSE 由 1.790 降到 1.173，负值预测占比由 55.4% 降到 0%。
  （旧结果取自原工程目录下的 `真实值与预测值对比.xlsx`。）
